    ...
    private void addButtonEvent() {
        shangButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                //1.边界处理, 当x0=0, 不能继续向上移动
                if (x0 == 0) {
                    System.out.println("0号图片在最上层,不能进行上移动");
                    return;
                }
                //2.交换0号图片和上方图片 (代码先抄下来,慢慢理解)
                datas[x0][y0] = datas[x0 - 1][y0];
                datas[x0 - 1][y0] = 0;
                x0 = x0 - 1;
                //3.编写并调用重绘的方法rePaintView()
                rePaintView();
                //4.判断是否成功,成功则禁用按钮
                if (isSuccess()) {
                    success();
        ...
        zuoButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                //1.边界处理,当y0=0,不能继续向左移动
                if (y0 == 0) {
                    System.out.println("0号图片在最左边,不能进行左移动");
                    return;
                }
                //2.交换0号图片和左方图片
                datas[x0][y0] = datas[x0][y0 - 1]; //用左边的图片,覆盖0号图片
                datas[x0][y0 - 1] = 0; //让左边的图片为0号图片
                y0 = y0 - 1; //修改索引
                //3.调用重绘的方法rePaintView()
                rePaintView();
                //4.判断是否成功,成功则禁用按钮
                if (isSuccess()) {
                    success();
